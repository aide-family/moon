...
var rootCmd = &cobra.Command{
	Use:   "moon",
	Short: "CLI for managing Moon monitor laurel Server",
	Long:  `The Moon Server CLI provides a command-line interface for managing and interacting with the Moon monitor laurel Server service.`,
	Run: func(cmd *cobra.Command, args []string) {
		fmt.Println("Welcome to the moon laurel service from Moon Monitor!")
		run(cfgPath)
	},
}

func init() {
	rootCmd.PersistentFlags().StringVarP(&cfgPath, "conf", "c", "./cmd/laurel/config", "Path to the configuration files")
}
