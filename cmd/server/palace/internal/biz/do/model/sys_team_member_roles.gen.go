...
type SysTeamMemberRole struct {
	UserID uint32 `gorm:"column:user_id;type:int unsigned;not null;uniqueIndex:idx__user_id__team_id__role_id,priority:1;comment:系统用户ID" json:"user_id"` // 系统用户ID
	TeamID uint32 `gorm:"column:team_id;type:int unsigned;not null;uniqueIndex:idx__user_id__team_id__role_id,priority:2;comment:团队ID" json:"team_id"`   // 团队ID
	RoleID uint32 `gorm:"column:role_id;type:int unsigned;not null;uniqueIndex:idx__user_id__team_id__role_id,priority:3;comment:团队角色ID" json:"role_id"` // 团队角色ID
