...
func NewHouYiConn(c *palaceconf.Bootstrap) (*HouYiConn, func(), error) {
	...
	default:
		return nil, nil, merr.ErrorNotification("暂不支持该网络类型：%s", houYiServer.GetNetwork())
	}
	// 退出时清理资源
	cleanup := func() {
		if houYiConn.rpcClient != nil {
			if err := houYiConn.rpcClient.Close(); err != nil {
				log.Errorw("关闭 houYi rpc 连接失败：", err)
			}
		}
		if houYiConn.httpClient != nil {
			if err := houYiConn.httpClient.Close(); err != nil {
