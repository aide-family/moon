...
func (l *cacheRepositoryImpl) GetUser(ctx context.Context, userID uint32) *model.SysUser {
	...
	if err := l.data.GetCacher().Client().Get(ctx, userCacheKey(userID)).Scan(&user); err != nil {
...
func (l *cacheRepositoryImpl) GetTeam(ctx context.Context, teamID uint32) *model.SysTeam {
	...
	if err := l.data.GetCacher().Client().Get(ctx, teamCacheKey(teamID)).Scan(&team); err != nil {
...
func (l *cacheRepositoryImpl) AppendUser(ctx context.Context, user *model.SysUser) {
	_ = l.data.GetCacher().Client().Set(ctx, userCacheKey(user.ID), user, 12*time.Hour).Err()
}

func (l *cacheRepositoryImpl) AppendTeam(ctx context.Context, team *model.SysTeam) {
	_ = l.data.GetCacher().Client().Set(ctx, teamCacheKey(team.ID), team, 12*time.Hour).Err()
}

func (l *cacheRepositoryImpl) GetUserTeamList(ctx context.Context, userID uint32) []*model.SysTeam {
	var teamIDs []uint32
	teamIDsStr, err := l.data.GetCacher().Client().Get(ctx, userTeamCacheKey(userID)).Result()
	if err != nil {
		return nil
	}
...
func (l *cacheRepositoryImpl) SyncUserTeamList(ctx context.Context, userID uint32) {
	...
	if len(teamIDs) > 0 {
		...
		if err := l.data.GetCacher().Client().Set(ctx, userTeamCacheKey(userID), string(teamIdsStr), 12*time.Hour).Err(); err != nil {
...
func (l *cacheRepositoryImpl) AppendDict(ctx context.Context, dict imodel.IDict, isBiz bool) {
	_ = l.data.GetCacher().Client().Set(ctx, dictCacheKey(ctx, dict.GetID(), isBiz), dict, 12*time.Hour).Err()
}

func (l *cacheRepositoryImpl) AppendDictList(ctx context.Context, dict []imodel.IDict, isBiz bool) {
	for _, item := range dict {
		_ = l.data.GetCacher().Client().Set(ctx, dictCacheKey(ctx, item.GetID(), isBiz), item, 12*time.Hour).Err()
	}
}
...
func (l *cacheRepositoryImpl) GetDict(ctx context.Context, id uint32, isBiz bool) imodel.IDict {
	if isBiz {
		...
		if err := l.data.GetCacher().Client().Get(ctx, dictCacheKey(ctx, id, isBiz)).Scan(&dict); err != nil {
