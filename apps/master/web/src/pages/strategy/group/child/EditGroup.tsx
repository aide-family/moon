...
const EditGroup: React.FC<EditGroupProps> = (props) => {
    ...
    const handleOnOk = () => {
        form.validate().then((values: GroupItem) => {
            if (initialValues && id) {
    ...
    const onEditGroup = (id: number, data: GroupCreateItem): Promise<Response> => {
    ...
    return <>
        <Modal visible={visible} title={props.title} onOk={handleOnOk} onCancel={onClose}>
            <Form form={form} layout="vertical">
                ...
                >
                    <Select
                        placeholder="请选择标签" mode="multiple"
                        options={[]} showSearch
                    />
                </Form.Item>
                <Form.Item label="备注" field="remark">
                    <Input.TextArea placeholder="请输入备注" maxLength={255} showWordLimit autoComplete="off"/>
                </Form.Item>
            </Form>
        </Modal>
    </>
