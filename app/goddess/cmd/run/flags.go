...
func (f *RunFlags) addFlags(c *cobra.Command, bc *conf.Bootstrap) {
	...
	c.PersistentFlags().StringVar(&f.Server.Name, "server-name", f.Server.Name, `Example: --server-name="goddess"`)
	...
	c.PersistentFlags().StringSliceVar(&f.metadata, "server-metadata", metadataStr, `Example: --server-metadata="tag=goddess" --server-metadata="email=aidecloud@163.com"`)
