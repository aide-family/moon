...
func newMySQLCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			var dsn string
			if database == "" {
				dsn = fmt.Sprintf("%s:%s@tcp(%s:%s)/?charset=utf8mb4&parseTime=True&loc=Local", user, url.QueryEscape(pass), host, port)
				conn, err := gorm.Open(mysql.Open(dsn), gormConfig)
				if err != nil {
					return fmt.Errorf("open mysql: %w", err)
				}
				database = "jade_tree_schema_tmp"
				...
				dsn = fmt.Sprintf("%s:%s@tcp(%s:%s)/%s?charset=utf8mb4&parseTime=True&loc=Local", user, url.QueryEscape(pass), host, port, database)
			}
