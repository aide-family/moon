...
func (f *RunFlags) addFlags(c *cobra.Command, bc *conf.Bootstrap) {
	...
	c.PersistentFlags().StringVar(&f.Server.Name, "server-name", f.Server.Name, `Example: --server-name="jade_tree"`)
	...
	c.PersistentFlags().StringSliceVar(&f.metadata, "server-metadata", metadataStr, `Example: --server-metadata="role=agent"`)
	c.PersistentFlags().StringVar(&f.environment, "environment", f.Environment.String(), `Example: --environment="DEV", --environment="PROD"`)
}
...
func (f *RunFlags) ApplyToBootstrap() error {
	...
	if len(f.configPaths) > 0 {
		...
		sourceOpts := make([]kconfig.Source, 0, len(f.configPaths)+1)
