...
func WireApp(serviceName string, bc *conf.Bootstrap, helper *log.Helper) ([]*kratos.App, func(), error) {
	...
	agentCommandDispatcher := impl.NewAgentCommandDispatcher()
	bizSSHCommand := biz.NewSSHCommand(sshCommand, commandAudit, sshOperator, machineInfoProvider, agentCommandDispatcher, helper)
	...
	servers := server.RegisterService(bc, httpServer, grpcServer, machineInfoReporterServer, healthService, sshCommandService, machineInfoService, probeTaskService)
