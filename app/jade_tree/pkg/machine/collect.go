...
func Collect(ctx context.Context) (*MachineInfo, error) {
	...
	if cpu != nil {
		...
		for _, p := range cpu.Processors {
			...
			for _, c := range p.Cores {
				...
				pbo.Cores = append(pbo.Cores, &MachineCPUCore{
					ID:              int32(c.ID),
...
func toMemoryInfo(info *ghw.MemoryInfo) *MachineMemory {
...
func getMountsByDisk() map[string][]*MachineDiskMount {
	result := make(map[string][]*MachineDiskMount)
	...
	for sc.Scan() {
		...
		result[diskName] = append(result[diskName], &MachineDiskMount{
...
func collectNetworkInfo() *MachineNetwork {
	n := &MachineNetwork{}
