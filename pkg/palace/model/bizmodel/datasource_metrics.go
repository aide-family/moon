...
type DatasourceMetric struct {
	AllFieldModel
	Name         string          `gorm:"column:name;type:varchar(255);not null;comment:指标名称;uniqueIndex:idx__name__category__datasource_id__deleted_at" json:"name"`
	Category     vobj.MetricType `gorm:"column:category;type:int;not null;comment:指标类型（对应prometheus四种数据类型）;uniqueIndex:idx__name__category__datasource_id__deleted_at" json:"category"`
	...
	DatasourceID uint32          `gorm:"column:datasource_id;type:int unsigned;not null;comment:所属数据源;uniqueIndex:idx__name__category__datasource_id__deleted_at" json:"datasource_id"` // 所属数据源
	// 更新时间
	DeletedAt soft_delete.DeletedAt `gorm:"column:deleted_at;type:bigint;not null;comment:删除时间;uniqueIndex:idx__name__category__datasource_id__deleted_at" json:"deleted_at"` // 删除时间
	// 标签数量
