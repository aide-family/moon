...
func (k *KafkaEvent) init() error {
	...
	default:
		return merr.ErrorNotificationSystemError("Unrecognized consumer group partition assignor: %s", k.c.GetStrategy())
	}
	...
	consumerGroup, err := sarama.NewConsumerGroup(k.c.GetBrokers(), k.c.GetGroupName(), config)
	if err != nil {
